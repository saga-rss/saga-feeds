...
schema.statics.updateFollowerCount = async function updateFollowerCount(
  id,
  amount
) {
  await this.findOneAndUpdate(
    { _id: id },
    { $inc: { followerCount: amount } }
  ).exec();
};

schema.statics.updateFavoriteCount = async function updateFavoriteCount(
  id,
  amount
) {
  await this.findOneAndUpdate(
...
schema.statics.createOrUpdateFeed = async function createOrUpdateFeed(
  discovery
) {
  const self = this;
  ...
  await Promise.map(discovery.feedUrls, async f => {
    ...
    if (
      !rss ||
      isAfter(new Date(rss.lastScrapedDate), subSeconds(new Date(), 30))
    ) {
