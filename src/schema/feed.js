...
const feedCreate = async (source, { feedUrl }, context) => {
  ...
  const feeds = await Promise.mapSeries(discovered.feedUrls, async feedUrl => {
    ...
    if (exists) {
      logger.debug(
        `This feed already exists, and does not need to be created`,
        {
          feedUrl
        }
      );
...
const feedSearch = async (
  ...
  const feeds = await context.models.feed
    .find()
...
const feedUnsubscribe = async (source, { feedId }, context) => {
  const subscription = await context.models.subscription.unsubscribe(
    feedId,
    context.user.sub
  );
  return subscription;
